...
//  PokPok3
//
//  Created by Dinaka Sadariskar on 14/12/21.
//
...
struct LocalNotificationManager {
    ...
    static private func scheduleNotifications(_ time: Date, userInfo: [AnyHashable : Any]) {
...
//          let svc: SettingsViewController = SettingsViewController(nibName: nil, bundle: nil)
//          let time = svc.timePicker.date


          let triggerDaily = Calendar.current.dateComponents([.hour, .minute], from: time)
...
//    }
    ...
    static func setNotification(_ time: Date, title: String, body: String, userInfo: [AnyHashable : Any]) {
        requestPermission()
        addNotification(title: title, body: body)
        scheduleNotifications(time, userInfo: userInfo)
    }
